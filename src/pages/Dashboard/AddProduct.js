...
const AddProduct = () => {
  ...
  const {isLoading, postSucces, isError, error} = useSelector(
  ...
  useEffect(() => {
    ...
    if (!isLoading && postSucces) {
      toast.success("Product Added...", {id: "addproduct"});
      dispatch(togglePostSucces());
      reset();
    }
    if (!isLoading && isError) {
      toast.error(error, {id: "addproduct"});
    }
  }, [isLoading, postSucces, isError, error]);
  const submit = (data) => {
    const product = {
      model: data.model,
      brand: data.brand,
